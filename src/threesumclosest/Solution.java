...
public class Solution {
	...
	public int closet = Integer.MAX_VALUE;
	public int sum = 0;
	public int threeSumClosest(int[] nums, int target) {
		...
		for(int t=0;t<nums.length-2;t++){
			...
			int j = nums.length-1;
			while(i<j){
				int mark = nums[t]+nums[i]+nums[j];
				int abs = Math.abs(mark-target);
				if(abs<closet){
					closet = abs;
					sum = mark;
				}
	...
	public static void main(String[] args) {
		...
		System.out.println(s.threeSumClosest(nums, 284));
		long endMili = System.currentTimeMillis();
		System.out.println("°æ time£∫ "+(endMili-startMili)+" ∫¡√Î °ø");
	}
}
